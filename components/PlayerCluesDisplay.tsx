...
interface PlayerCluesDisplayProps {
  playerId: string;
}

export default function PlayerCluesDisplay({ playerId }: PlayerCluesDisplayProps) {
  const { data, error, isLoading, mutate } = useSWR<{ clues: Clue[] }>(
    `/api/gm/players/${playerId}/clues`,
    fetcher,
    {
    ...
    return (
      <div className="bg-gray-800 rounded-lg dynamic-card-padding card-container text-center">
        ...
        <p className="dynamic-text-base" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>
          No clues have been released for this player yet.
        </p>
      </div>
    );
  }
