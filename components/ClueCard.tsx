...
// - Updated: Added markdown rendering support for backstory section to allow line breaks and text formatting
// - Updated: Footer badges (Legitimacy and Confidentiality) now use global clue-badge class with green background and white text
"use client";
...
export default function ClueCard({ clue }: ClueCardProps) {
  ...
  return (
    <div className="bg-gray-800 rounded-lg overflow-hidden card-container-thick">
        ...
        >
          ...
          >
            ...
            <div className="flex-1">
              <h3
                className="dynamic-text-xl font-bold"
                style={{ color: "white" }}
              >
                {clue.title}
              </h3>
      ...
      >
        <div className="dynamic-card-padding">
          ...
          <div style={{ marginBottom: "clamp(1rem, 3vw, 1.5rem)" }}>
            <div className="flex justify-between dynamic-text-base mb-2">
              ...
              >
                {
                  CONFIDENCE_DISPLAY[
                    clue.confidenceLevel as keyof typeof CONFIDENCE_DISPLAY
                  ]
                }
              </span>
            </div>
            <div
              className="bg-gray-900 rounded-full overflow-hidden"
              style={{ height: "clamp(0.5rem, 1.5vw, 0.75rem)" }}
            >
              <div
                className={`bg-blue-500 h-full ${getConfidenceWidth(
          ...
          {clue.supportingIntel && (
            <div
              className="bg-gray-900 rounded dynamic-card-padding card-container"
              style={{ marginBottom: "clamp(1rem, 3vw, 1.5rem)" }}
            >
              <h4
                className="dynamic-text-base font-semibold mb-2"
                style={{ color: "rgba(255, 255, 255, 0.6)" }}
              >
                SUPPORTING INTEL:
              </h4>
            ...
            <div style={{ marginBottom: "clamp(1rem, 3vw, 1.5rem)" }}>
              ...
              >
                {clue.takeaways.map((takeaway, idx) => (
                  <li
                    key={idx}
                    className="dynamic-text-base flex"
                    style={{
                      gap: "clamp(0.5rem, 1.5vw, 0.75rem)",
      ...
      {/* Footer */}
      <div
        className="border-t card-separator dynamic-card-padding flex"
        style={{ gap: "clamp(0.75rem, 2vw, 1rem)" }}
      >
        <span className="clue-badge">
          {
            LEGITIMACY_DISPLAY[
              clue.legitimacy as keyof typeof LEGITIMACY_DISPLAY
            ]
          }
        </span>
        <span className="clue-badge">
          {
            CONFIDENTIALITY_DISPLAY[
              clue.confidentiality as keyof typeof CONFIDENTIALITY_DISPLAY
            ]
          }
        </span>
      </div>
